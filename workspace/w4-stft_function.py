...
def main(inputFile = '../sounds/piano.wav', window = 'hamming', M = 1024, N = 1024, H = 512):
	...
	plt.pcolormesh(frmTime, binFreq, np.transpose(mX[:,:int(N*maxplotfreq/fs+1)]), cmap='jet')
	plt.xlabel('time (sec)')
	...
	frmTime = H*np.arange(numFrames)/float(fs)
	binFreq = fs*np.arange(N*maxplotfreq/fs)/N
	plt.pcolormesh(frmTime, binFreq, np.transpose(np.diff(pX[:,:int(N*maxplotfreq/fs+1)],axis=1)), cmap='jet')
	plt.xlabel('time (sec)')
	plt.ylabel('frequency (Hz)')
